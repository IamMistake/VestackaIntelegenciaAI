...
class Squares(Problem):
    def __init__(self, initial, house):
        ...
        self.moves = {
            "Pomesti kvadratche , gore": (0, 1),
            "Pomesti kvadratche , dolu": (0, -1),
            "Pomesti kvadratche , levo": (-1, 0),
            "Pomesti kvadratche , desno": (1, 0)
        }
    ...
    def successor(self, state):
        ...
        for tuplce in state:
            ...
            for move in self.moves.keys():
                ...
                if not Squares.check_valid(next_state):
                    # br += 1
                    continue

                name = move.split(",")
                name = name[0] + str(br + 1) + name[1]
                succ[name] = next_state
