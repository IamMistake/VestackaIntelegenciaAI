    ...
    def successors(self, state):
        ...
        for tupce in state:
            ...
            for drugi in state:
                if brojac != brojac2:
                    listce = list(tupce)
                    last = listce[len(listce) - 1]

                    if len(drugi) > 0:
                        top = drugi[len(drugi) - 1]
                        if top < last:
                            brojac2 += 1
                            continue
                    ...
                    next_state[brojac] = tuple(listce)
                    next_state[brojac2] = tuple(tmp)

                    # print(next_state)
                    name = "MOVE TOP BLOCK FROM PILLAR " + str(brojac + 1) + " TO PILLAR " + str(brojac2 + 1)
...
def my_mapper(element):
    if element:
        return tuple(map(int, element.split(",")))
    else:
        return ()

if __name__ == "__main__":
    starting_pillars = tuple(map(my_mapper, (input().split(";"))))
    end_pillars = tuple(map(my_mapper, (input().split(";"))))
    # print(starting_pillars)
    # print(end_pillars)

    problem = Pillars(starting_pillars, end_pillars)
    res = breadth_first_graph_search(problem)
    if res:
        print("Number of action " + str(len(res.solution())))
        print(res.solution())
    else:
